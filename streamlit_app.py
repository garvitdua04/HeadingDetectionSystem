...
#                 system.train(samples_per_class=10000, optimise=False)
...
#                 st.write(f"{model_type.title()}: {stats['accuracy']:.3f}")
...
#         result = system.process_pdf(input_path, None)
...
#                     st.write(f"{indent}**{level}**: {text} *(Page {page})*")
#             else:
...
#                     label="Download JSON",
#                     data=json_str,
#                     file_name=f"{uploaded_file.name}_headings.json",
